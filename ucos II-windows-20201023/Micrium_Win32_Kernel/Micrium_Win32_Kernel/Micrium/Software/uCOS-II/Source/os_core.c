...
void Sched_RMS() {
	OS_TCB *ptcb;
	ptcb = OSTCBList;   //从头开始遍历，找就绪态中优先级最高的，即周期最短的
	INT32U min_p;
	...
	//开始遍历所有的TCB
	while (ptcb->OSTCBPrio < OS_TASK_IDLE_PRIO - 2) {  //注意，除了63任务外，还有62、61任务
		OS_ENTER_CRITICAL();
		//如果这个任务是就绪态的
		if (ptcb->OSTCBDly == 0) {
			task_info = (tcb_ext_info*)ptcb->OSTCBExtPtr;

			OS_EVENT* device = task_info->resource;
			if (device != (OS_EVENT*)0 && task_info->rest_c == task_info->c) {
				//需要请求资源
				int remain_resource = OSSemAccept(device);
				//printf("\nRemain is %d.", remain_resource);
				if (remain_resource == 0) {
					...
					//请求成功，返还
					OSSemPost(device);
					//printf("\nRemain are %d.", remain_resource);
				}
			}
			INT32U task_p = task_info->p;

			//如果这个任务的p比min_p小，就进行替换
			if (task_p < min_p) {
				min_p = task_p;
	...
	if (OSPrioHighRdy != OS_TASK_IDLE_PRIO) {
		task_info = (tcb_ext_info*)OSTCBPrioTbl[OSPrioHighRdy]->OSTCBExtPtr;
		OS_EVENT* device = task_info->resource;
		if (device != (OS_EVENT*)0 && task_info->rest_c == task_info->c ) {
			if (!(tcb_ext_info*)OSTCBCur->OSTCBExtPtr) {
				//需要请求资源
				OSSemAccept(device);
			}
			else {
...
void Sched_EDF() {
	...
	OS_ENTER_CRITICAL();
	min_ddl = 1000000;//MAX
	OSPrioHighRdy = OS_TASK_IDLE_PRIO;//空闲任务的prio
	OS_EXIT_CRITICAL();
	...
	while (ptcb->OSTCBPrio < OS_TASK_IDLE_PRIO - 2) {  //注意，除了63任务外，还有62、61任务
		...
		if (ptcb->OSTCBDly == 0) {
			...
			INT32U task_ddl = task_info->rest_p;
			//如果这个任务的ddl比min_ddl小，就进行替换
			if (task_ddl < min_ddl) {
				min_ddl = task_ddl;
				OSPrioHighRdy = ptcb->OSTCBPrio;
			}
